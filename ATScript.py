...
class Main:
    ...
    def calc_factor(self):
        ...
        for ind in range(0, self.numOfIterations * 2, 2):
            ...
            calc_max_val_pe = max(np.multiply(self.factorPE, self.MatrixPEAll[ind + 1][:]))  # calculation max value of
            # PE for each iteration
            calc_min_val_pe = min(np.multiply(self.factorPE, self.MatrixPEAll[ind + 1][:]))  # calculation min value of
            # PE for each iteration
            calc_max_val_ptpvcmd = max(self.MatrixPEAll[ind][:])  # calculation max value of PTPVCMD for each iteration

            calc_max_val_icmd = max(self.MatrixICMDAll[ind + 1][:])  # calculation max value of ICMD for each iteration
            calc_min_val_icmd = min(self.MatrixPEAll[ind + 1][:])  # calculation min value of ICMD for each iteration
        ...
        self.max_vlocity = int(max_val_ptpvcmd_vs_pe)  # for plot setling
...
class Record(Main):
    ...
    def record_all_profile(self, iteration):
        ...
        try:
            ...
            recall_file = ""
            try:
                recall_file = open(self.dir_iter + '\RecordAll.txt', "w")
    ...
    def record_dec_profile(self, iteration):
        ...
        try:
            ...
            for x in range(int(len(columns) / num_of_rec_objects)):
                arr_ptpvcmd.insert(x, float(columns[inx_ptpvcmd]))
                arr_pe.insert(x, float(columns[inx_pe]))
                arr_icmd.insert(x, float(columns[inx_icmd]))

                inx_ptpvcmd += num_of_rec_objects
                inx_pe += num_of_rec_objects
                inx_icmd += num_of_rec_objects
            ...
            recset_file = ""
            try:
                recset_file = open(self.dir_iter + '\RecordDec.txt', "w")
                recset_file.write(get)
            finally:
                recset_file.close()
...
class Plot(Main):
    ...
    def plot_rec_dec_pe(self):
        ...
        plt.xlim([0, (self.sample_value * 31.25 * self.time_rec) / 1000])
        plt.grid()
        plt.title('PTPVCMD(' + self.traj_array[1] + '[rpm/s]' + ')' + ' *factor(' + str(self.factor_ptpvcmd_pe) + ')'
                  + ' & PE(' + str(self.factorPE) + ')')
    ...
    def plot_rec_dec_pe_settling(self):
        ...
        # plt.xlim([-100, 1950])
        plt.xlim([0, (self.sample_value * 31.25 * self.time_rec) / 1000])
        plt.ylim([-float(self.max_vlocity*self.factor_ptpvcmd_pe*0.5),
                  float(self.max_vlocity*self.factor_ptpvcmd_pe*0.5)])
        plt.grid()
